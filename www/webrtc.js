...
let channel =
  {}; /* keep track of the peers Info in the channel, indexed by peer_id (aka socket.io id) */
  ...
  signalingSocket.on("disconnect", function () {
    for (let peer_id in peerMediaElements) {
      document
        ...
        .removeChild(peerMediaElements[peer_id].parentNode);
      resizeVideos();
    }
  ...
  signalingSocket.on("addPeer", function (config) {
    ...
    peerConnection.onaddstream = function (event) {
      ...
      for (let peerId in channel) {
        const videoPeerName = document.getElementById(
          peerId + "_videoPeerName"
        );
        ...
        const videoAvatarImg = document.getElementById(
          peerId + "_videoEnabled"
        );
        ...
        const audioEnabledEl = document.getElementById(
          peerId + "_audioEnabled"
        );
        const audioEnabled = channel[peerId]["userData"]["audioEnabled"];
        if (audioEnabledEl) {
          audioEnabledEl.className =
            "audioEnabled icon-mic" + (audioEnabled ? "" : "-off");
        }
      }
    };
    ...
    dataChannels[peer_id] = peerConnection.createDataChannel(
      "CamConnect__data_channel"
    );
  ...
  navigator.mediaDevices
    ...
    .then((stream) => {
      ...
      navigator.mediaDevices.enumerateDevices().then((devices) => {
        App.videoDevices = devices.filter(
          (device) =>
            device.kind === "videoinput" && device.deviceId !== "default"
        );
        App.audioDevices = devices.filter(
          (device) =>
